...
- [x] 1.1 Criar `src/api/decks.ts`: `listDecks()`, `createDeck(input)`,
...
- [x] 1.2 Criar `src/api/cards.ts`: `listCards(deckId, { page, pageSize,
...
- [x] 3.2 Criar `cardInputFromValues`/`EMPTY_CARD_VALUES` (mesmo papel de
...
- [x] 4.4 Navegar para `DeckDetail` ao tocar em um baralho da lista —
      verificar com teste de componente/navegação

## 5. Detalhe do baralho (`DeckDetailScreen`)

- [x] 5.1 Criar `src/screens/DeckDetailScreen.tsx`: carrega `getDeck(id)`
      ...
      teste de componente
- [x] 5.2 Exibir nome, idiomas, total de cards, data de criação e os
      quatro blocos de contagem por status — verificar com teste de
      componente
- [x] 5.3 Editar (nome/idiomas) e excluir o baralho, com confirmação antes
      de excluir — verificar com teste de componente os dois fluxos
- [x] 5.4 Lista de cards paginada com `FlatList` (`onEndReached` carrega a
      ...
      a primeira
- [x] 5.5 Busca por palavra (debounce de 300ms) e filtro por status,
      ...
      debounce)
- [x] 5.6 Cada card na lista mostra selo de status (mapeamento fixo
      ...
      inteira
- [x] 5.7 Estado de "nenhum card encontrado" para busca/filtro sem
...
- [x] 6.1 Criar `src/screens/AddCardScreen.tsx`, recebendo o `deckId` por
...
- [x] 8.1 `npx tsc --noEmit` passa sem erros
- [x] 8.2 `npm run lint` passa sem erros
- [x] 8.3 `npm test` passa, cobrindo `api/decks`, `api/cards`,
...
- [x] 8.5 `openspec validate add-decks-and-cards-screens --type change
      --strict` passa
