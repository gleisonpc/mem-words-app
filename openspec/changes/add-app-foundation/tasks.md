...
- [x] 1.1 Criar o projeto com `npx create-expo-app@latest . --template
...
- [x] 1.2 Configurar `app.json`/`app.config.ts`: nome do app, `slug`,
...
- [x] 1.3 Adicionar `eas.json` com os perfis `development` (dev client,
...
- [x] 1.4 Habilitar TypeScript estrito em `tsconfig.json`
      (`"strict": true"`) — verificar `npx tsc --noEmit` sem erros no
      projeto gerado
- [x] 1.5 Configurar lint (oxlint, mesma ferramenta do
      `mem-words-frontend`) com script `lint` no `package.json` — verificar
      `npm run lint` roda sem erro de configuração
- [x] 1.6 Criar a estrutura de pastas `src/{api,theme,navigation,screens}`
...
- [x] 2.1 Portar os tokens de cor do `mem-words-frontend`
...
- [x] 2.2 Verificar manualmente o contraste de cada par
...
- [x] 2.3 Criar `src/theme/ThemeProvider.tsx`: lê `useColorScheme()`,
...
- [ ] 2.4 Verificar que a escolha persiste: fechar e reabrir o app (no
      Expo Go) com um tema fixado mantém o mesmo tema — **pendente:
      requer dispositivo físico do usuário (sem Mac/simulador aqui)**
- [x] 2.5 Adicionar escalas de espaçamento, tamanho de fonte, peso de
      fonte e raio de borda em `src/theme/tokens.ts`, com os mesmos
      degraus (ou equivalentes) do `mem-words-frontend`
- [x] 2.6 Ler `AccessibilityInfo.isReduceMotionEnabled()` e o evento
...
- [x] 3.2 Criar `src/api/ApiError.ts`, adaptando
      ...
      `timeout`
- [x] 3.3 Criar `src/api/client.ts` com `request(path, { method, body,
...
- [ ] 4.2 Criar `src/navigation/RootNavigator.tsx` com um
      ...
      verificar navegação manual entre as duas no Expo Go — **código
      implementado; verificação em dispositivo pendente (sem Mac/simulador
      aqui, delegada ao usuário)**
- [ ] 4.3 Configurar `linking` no `NavigationContainer` com
      ...
      open` ou o próprio Expo Go) e chegando na tela correta — **código
      implementado; verificação em dispositivo pendente**
- [ ] 4.4 Adicionar uma tela de fallback (`NotFoundScreen`) para rota não
      ...
      link para uma rota inexistente — **código implementado; verificação
      em dispositivo pendente**
...
- [ ] 5.1 Criar `src/screens/DiagnosticsScreen.tsx`: chama `checkHealth()`
      ...
      **código implementado; verificação visual em dispositivo pendente**
- [ ] 5.2 Criar `src/screens/HomeScreen.tsx` provisória, com um link para
      a tela de diagnóstico — verificar navegação — **código
      implementado; verificação em dispositivo pendente**
- [ ] 5.3 Testar os três cenários no dispositivo físico via Expo Go:
      ...
      visualmente distintos — **pendente: requer dispositivo físico do
      usuário**

## 6. Verificação final

- [x] 6.1 `npx tsc --noEmit` passa sem erros
...
- [ ] 6.3 `npx eas build --profile development --platform ios` conclui
      ...
      **bloqueado: requer conta EAS e Apple Developer Program, que ainda
      não existem (ver design.md, "Non-Goals")**
- [x] 6.4 `openspec validate --specs` passa para as quatro capabilities
      desta change
